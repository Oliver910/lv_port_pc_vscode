 ...
 *   STATIC FUNCTIONS
 **********************/
void hal_mouse_read(lv_indev_drv_t *indev_drv, lv_indev_data_t *data) {

  if (vnc_mouse_read(indev_drv, data) == 1) {
    update_mouse(data->point.x, data->point.y, data->state);
  }
  mouse_read(indev_drv, data);
}
...
static void hal_init(void) {
   ...
   * position and state*/
  indev_drv_1.read_cb = hal_mouse_read;
